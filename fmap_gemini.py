...
class GeminiInterpretRequest(BaseModel):
    prompt: str = Field(..., description="User prompt/instruction for interpretation")
    context: Optional[Dict[str, Any]] = Field(None, description="Structured context (analysis, time series, etc.)")
    model: Optional[str] = Field(None, description="Gemini model name (optional). Defaults to GEMINI_MODEL env var.")
    temperature: Optional[float] = Field(None, ge=0.0, le=2.0, description="Optional generation temperature.")
...
@app.post("/ai/gemini")
def ai_gemini(req: GeminiInterpretRequest):
    """
    Server-side Gemini call. Keeps API key off the browser.

    Requires Render env var: GEMINI_API_KEY
    ...
    model = (req.model or os.getenv("GEMINI_MODEL") or "gemini-2.5-flash").strip()

    # Build a single text prompt (keeps things simple and compatible with JSON mode off)
    prompt = (req.prompt or "").strip()
    if not prompt:
        raise HTTPException(status_code=400, detail="prompt is required.")

    # Keep request small; avoid blowing memory/logs
    ctx_text = ""
    if req.context is not None:
        try:
            ctx_text = json.dumps(req.context, ensure_ascii=False, indent=2)
        except Exception:
            ctx_text = json.dumps(req.context)
    ...
    try:
        ...
        out_text = None
        try:
            cand0 = (js.get("candidates") or [None])[0] or {}
        ...
        return {"text": out_text or "", "model": model, "latency_seconds": round(dt, 3)}
    except urllib.error.HTTPError as e:
        # Gemini returns JSON error bodies; surface them safely
        try:
            err_body = e.read().decode("utf-8", errors="replace")
        except Exception:
            err_body = str(e)
        raise HTTPException(status_code=502, detail=f"Gemini HTTPError {e.code}: {err_body}")
    except Exception as e:
